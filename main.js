...
function draw() {
    ...
    gl.uniform2fv(shProgram.iUserPoint, [0.0, 0.0]); //giving coordinates of user point
    ...
    let tranSphere = pear(3.0, 2.0, map(userPointCoord.x,0,1,0,3.0), map(userPointCoord.y,0,1,0,Math.PI*2));
    console.log(userPointCoord)
    console.log(tranSphere)
    gl.uniform3fv(shProgram.iUP, [tranSphere.x, tranSphere.y, tranSphere.z]);
    // gl.uniform3fv(shProgram.iUP, [-1.12*0.05, 0.82*0.05, 1.95*0.05]);
    sphere.DrawSphere();
}
...
function CreateNormalsData() {
    ...
    while (i <= a) {
        while (j <= Math.PI * 2) {
            let v1 = pear(a, b, i, j)
            let v2 = pear(a, b, i + 0.1, j)
            let v3 = pear(a, b, i, j + 0.1)
            let v4 = pear(a, b, i + 0.1, j + 0.1);
...
function pear(a, b, i, j) {
    let x = r(a, b, i) * Math.sin(j)
    let y = r(a, b, i) * Math.cos(j)
    let z = i
    return { x: x, y: y, z: z }
}
function r(a, b, i) {
    return (i * Math.sqrt(i * (a - i)) / b)
}
...
function initGL() {
    ...
    surface.NormalBufferData(CreateNormalsData());
    LoadTexture();
